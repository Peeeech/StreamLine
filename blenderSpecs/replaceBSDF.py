    ...
    if material.use_nodes and material.node_tree:
        ...
        if image_texture_node and material_output_node:
            for link in material.node_tree.links:
                if link.to_node == diffuse_bsdf_node:
                    material.node_tree.links.remove(link)
                    
            material.node_tree.links.new(image_texture_node.outputs['Color'], diffuse_bsdf_node.inputs['Color'])
            material.node_tree.links.new(image_texture_node.outputs['Alpha'], mix_shader.inputs['Fac'])
            material.node_tree.links.new(transparent_shader.outputs['BSDF'], mix_shader.inputs[1])
            material.node_tree.links.new(diffuse_bsdf_node.outputs['BSDF'], mix_shader.inputs[2])
            ...
            print(f"Material '{material.name}' has a transparent texture.")
