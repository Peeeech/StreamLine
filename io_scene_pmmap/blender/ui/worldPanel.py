...
"""
Main low-level data panel for TTYD-Blender

For primary layout, ctrl+f to "jump"
"""
...
class Sampler(bpy.types.PropertyGroup):
    ...
    texBlendMode: bpy.props.IntProperty() #type: ignore
...
class TTYD_OT_add_sampler(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        sampler.texCoord.scaleX = 1
        sampler.texCoord.scaleY = 1
...
class TTYD_OT_sync_joint_anim_pivot_from_loc(bpy.types.Operator):
    bl_idname = "ttyd.sync_joint_anim_pivot_from_loc"
    bl_label = "Sync Pivot from Location"
    bl_description = "Set anim_pivot to the joint object's current location"
    bl_options = {'UNDO'}
    ...
    def execute(self, context):
        ...
        loc = joint.location
        track.anim_pivot = (float(loc.x), float(loc.y), float(loc.z))
...
class TTYD_OT_rebuild_local_ir(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #Ensure no parent inverse matrix is messing up XYZ in TTYD-space by resetting the matrix_basis
        ...
        try:
            ...
            for tri in eval_mesh.loop_triangles:
                ...
                for li in loops:
                    ...
                    col_val = tuple(_read_color_value(col_layer[li])) if col_layer and ca.domain == 'CORNER' else tuple(_read_color_value(col_layer[loop.vertex_index])) if col_layer else None #type: ignore
                    ...
                    if key in vertex_map:
                        ...
                        if col_layer:
                            if ca.domain == 'CORNER': #type: ignore
...
class TTYD_OT_stripify_mesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if props.isCamRoadRegion:
            self.report({'ERROR'}, "Mesh is a CamRoad Region, cannot stripify.")
            return {'CANCELLED'}

        # ---- Ensure IR exists ----
        if not props.local_vertices:
            self.report({'ERROR'}, "Local vertices. Run Rebuild Local IR first.")
            return {'CANCELLED'}
        
        if not props.local_tri_indices:
            self.report({'ERROR'}, "Triangle indices. Run Rebuild Local IR first.")
            return {'CANCELLED'}

        # ---- Parse triangle index buffer ----
        try:
            tri_indices = list(map(int, props.local_tri_indices.split(",")))
        except Exception:
            self.report({'ERROR'}, "Invalid triangle index buffer.")
            return {'CANCELLED'}

        if len(tri_indices) < 3 or len(tri_indices) % 3 != 0:
            self.report({'ERROR'}, "Triangle index buffer is invalid.")
            return {'CANCELLED'}

        # ---- Build tris for stripifier ----
        tris = [
        ...
        # ---- Run stripifier ----
        all_strips = strip.stripify(tris)

        _clear_collection(props.local_primitives)

        for strip_indices in all_strips:
            prim = props.local_primitives.add()
            prim.opcode = GX_TRIANGLE_STRIP
            prim.indices = ",".join(map(str, strip_indices))

        props.ir_dirty = False

        self.report(
            {'INFO'},
            f"Stripified IR: {len(tris)} tris : {len(all_strips)} strip indices"
        )
...
class TTYD_OT_set_active_camroad_object(bpy.types.Operator):
    bl_idname = "ttyd.set_camroad_object"
    bl_label = "Simulate Camera Roads w/Selected"
    bl_description = "Use arrow-keys to move, and 'Esc' to cancel"
    bl_options = {'REGISTER', 'UNDO'}
                ...
                for region in area.regions:
                    if region.type == 'WINDOW':
                        with context.temp_override(area=area, region=region):
                            bpy.ops.view3d.move_active_with_arrows(
...
class TTYDWorldMeshProperties(bpy.types.PropertyGroup):
    ...
    local_tri_indices: bpy.props.StringProperty(name="Triangle Indices")  # comma-separated #type: ignore
...
class TTYDJointAttributes(bpy.types.PropertyGroup):
    origin_offset: bpy.props.FloatVectorProperty(size=3) #type: ignore #anim_pivot pushed to here for obj-specific I/O purposes
...
#region: 'class_ttyd_world_texture'
...
#region: 'class_ttyd_world_material'
class TTYDEmptyMaterialProperties(bpy.types.PropertyGroup):
    """
    First four properties are appended to materialEmpty for Blender Preview/Navigation purposes
    """

    # Bool to show the following two props in the panel
    showBlenderData: bpy.props.BoolProperty(default=True) #type: ignore

    # (Wrapper) Shows the created draw_mode(s) for the selected material
    materialRefs: bpy.props.CollectionProperty(
        type=TTYDEmptyMainMaterialRef
        ) #type: ignore
    
    # (Wrapper) Shows what all meshes currently reference the material TODO: update to sync (depsgraph? or manual?)
    ...
    # Material Color
    """     The color will be split into two sections; we'll make a nicer Colorwheel for UX, but the `color`
        op here will stay purely in 0-255 range for simpler export. All srgb math will happen automatically
        upon syncing the color wheel (if desired)
    """
    color: bpy.props.IntVectorProperty(size=4) #type: ignore
    colorWheel: bpy.props.FloatVectorProperty(
        name="Material Color",
        subtype='COLOR',
        size=4,
        min=0.0,
    ...
    # Material Source (Material Color / Vertex Colors [baked into geometry])
    ...
    blendMode: bpy.props.EnumProperty(
        ...
        items=[
            ...
            (BLENDMODE.FULL, "Full", "Alpha subtracted with translucency"),
        ]
    ) #type: ignore

    # Number of Textures directly reflects Sampler Count, they should always match
    ...
    # Unused(?) value, potentially able to animate transparency by driving the Red channel of the RGBA tuple
    blendAlphaModulationR: bpy.props.IntVectorProperty(size=4) #type: ignore

    # (Wrappers) independent data types stored inside material
...
class TTYDJointAnimTrack(bpy.types.PropertyGroup):
    ...
    anim_pivot: bpy.props.FloatVectorProperty(size=3) #type: ignore
...
#region: 'class_ttyd_world_animation'
...
class TTYDWorldPanel(bpy.types.Panel):
    """
    ========================================================================================================================
    jump ---- TTYD World foundation panel / authoritative world-state UI

    Purpose:
        Handles the core storage + panel access for DMD-related world data used by import/export and higher-level UI.

    Navigation:
        Search the following section handles with Ctrl+F:

        class_ttyd_world_empty        -> ttyd_world_empty
        class_ttyd_world_mesh         -> ttyd_world_mesh
        class_ttyd_world_light        -> ttyd_world_light
        class_ttyd_world_material     -> ttyd_world_material
        class_ttyd_world_texture      -> ttyd_world_texture
        class_ttyd_world_animation    -> ttyd_world_animation
        class_ttyd_world_curve        -> ttyd_world_curve

    Notes:
        This file is treated as a stable foundation layer. Higher-level UX panels may derive draw logic from this,
        but this remains the main reference point for low-level DMD state.
    ========================================================================================================================
    """
    ...
    def draw(self, context):
        ...
        if obj.type == 'EMPTY':
            ...
            if props.isLight:
                ...
                layout.prop(lightProps, "enableFlags"), #True if bytes are '0x12d60d8', has been True in all testing so far #type: ignore
            ...
            if props.isAnimation:
                ...
                if animProps.joint:
                    ...
                    for i, track in enumerate(jt.tracks):
                        ...
                        pivotrow = refBox.row(align=True)
                        pivotrow.prop(track, "anim_pivot", text="Pivot")
                        op = pivotrow.operator("ttyd.sync_joint_anim_pivot_from_loc", text="", icon="SNAP_ON")
                        op.index = i
