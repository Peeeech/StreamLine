...
bl_info = {
	"name": "Paper Mario DMD Map Exporter",
	"author": "Peeech (forked from PistonMiner (Linus S.))",
	"version": (1, 1, 0),
	"blender": (4, 0, 0),
	"location": "File > Import-Export",
	"description": "Import-Export as Paper Mario DMD Map Data File",
...
class ExportTTYDMap(bpy.types.Operator, ExportHelper):
	...
	#                       syntax is right.
	filter_glob: StringProperty(default="d;c;t", options={'HIDDEN'}) #type: ignore

	world_name: StringProperty(default="") #type: ignore

	collapse_hit: bpy.props.BoolProperty(
		name="Collapse Hit Data",
		description="Collapse hit data into purely position data, removing normals and UVs to save space",
		default=True
	) #type: ignore

	texture_root: EnumProperty(
		name="Texture Collection",
		items=enum_collections,
		default=0
	) #type: ignore
	material_root: EnumProperty(
			name="Material Collection",
			items=enum_collections,
			default=1
		) #type: ignore
	light_root: EnumProperty(
	...
	cam_root: EnumProperty(
			name="Camera Road Collection",
			items=enum_collections,
			default=5
		) #type: ignore
	anim_root: EnumProperty(
	...
	def execute(self, context):
		...
		texture_root = find_collection_by_name(context.scene, self.texture_root)
		material_root = find_collection_by_name(context.scene, self.material_root)
		light_root = find_collection_by_name(context.scene, self.light_root)
		map_root = find_collection_by_name(context.scene, self.map_root)
		hit_root = find_collection_by_name(context.scene, self.hit_root)
		cam_root = find_collection_by_name(context.scene, self.cam_root)
		anim_root = find_collection_by_name(context.scene, self.anim_root)
		...
		if len(collection_list) != len(set(collection_list)):
			self.report({'ERROR'}, "Light/Map/Hit/Camera Road collections must not be the same!")
			return {'CANCELLED'}

		settings["collapse_hit"] = self.collapse_hit
		settings["texture_root"] = texture_root
		settings["material_root"] = material_root
		settings["light_root"] = light_root
		settings["map_root"] = map_root
		settings["hit_root"] = hit_root
		settings["cam_root"] = cam_root
		settings["anim_root"] = anim_root

		settings["axis_conversion_matrix"] = axis_conversion(
			to_forward='-Z',
			to_up='Y'
		).to_4x4()
		return export_ttyd.export(context, settings, self.world_name)
