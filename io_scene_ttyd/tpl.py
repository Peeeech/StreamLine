...
class TplTexture:
	def __init__(self):
		...
		# ---- new per-texture sampler/header fields ----
		self.wrap_s = 1
		self.wrap_t = 1
		self.min_filter = 1
		self.mag_filter = 1
		self.lod_bias = 0.0
		self.edge_lod_enable = 0
		self.min_lod = 0
		self.max_lod = 0
	...
	def link(self, linker):
		...
		# data_addr via relocation at 0x08 (keep Piston logic)
		# wrap + filters
		struct.pack_into(">L", texture_header_data, 0x0C, int(self.wrap_s))
		struct.pack_into(">L", texture_header_data, 0x10, int(self.wrap_t))
		struct.pack_into(">L", texture_header_data, 0x14, int(self.min_filter))
		struct.pack_into(">L", texture_header_data, 0x18, int(self.mag_filter))

		# lod_bias + lod bytes
		struct.pack_into(">f", texture_header_data, 0x1C, float(self.lod_bias))
		struct.pack_into(">B", texture_header_data, 0x20, 1 if self.edge_lod_enable else 0)
		struct.pack_into(">B", texture_header_data, 0x21, int(self.min_lod) & 0xFF)
		struct.pack_into(">B", texture_header_data, 0x22, int(self.max_lod) & 0xFF)
		# 0x23 padding left as 0

		# image data (keep Piston’s alignment & relocation flow)
		texture_data_blob_name = "texture_data:" + str(texture_id)
		texture_data = self.texture_data.copy()
		pad = (-len(texture_data)) & 31   # 0..31 bytes
		texture_data.extend(b"\x00" * pad)

		linker.add_blob(texture_data_blob_name, texture_data)
		linker.place_blob_in_section(texture_data_blob_name, "texture_data")

		linker.add_relocation(texture_header_blob_name, 0x08, texture_data_blob_name)
	...
	@staticmethod
	def from_world_texture_props(blender_image, tex_props, idx, *, quality=False, format_override=None):		
		"""
		tex_props is your TTYDEmptyTextureProperties instance.
		"""
		texture = TplTexture()

		# Dimensions: prefer props if set, else image.size
		w, h = blender_image.size
		texture.size = (int(tex_props.width) or int(w), int(tex_props.height) or int(h))

		# Wrap/filter/lod: directly from props
		texture.wrap_s = int(tex_props.wrap_s)
		texture.wrap_t = int(tex_props.wrap_t)
		texture.min_filter = int(tex_props.min_filter)
		texture.mag_filter = int(tex_props.mag_filter)
		texture.lod_bias = float(tex_props.lod_bias)
		texture.edge_lod_enable = bool(tex_props.edge_lod_enable)
		texture.min_lod = int(tex_props.min_lod)
		texture.max_lod = int(tex_props.max_lod)

		# Format:
		# - If you want “forced format”, set props.format to a valid ID (0,1,2,...,14)
		# - Else use format_override, else auto-detect via your PIL logic.
		fmt_name = None
		if format_override is not None:
			fmt_name = format_override
		else:
			TPL_FORMAT_NAMES_BY_ID = {v: k for k, v in TPL_FORMAT_IDS.items()}

			fmt_id = int(tex_props.format)
			fmt_name = TPL_FORMAT_NAMES_BY_ID.get(fmt_id)

		fmt_name, encoded = encode.encode_blender_image(blender_image, idx, fmt=fmt_name, quality=quality)
		texture.format = fmt_name
		texture.texture_data = bytearray(encoded)
		return texture
